...
if len(sys.argv) != 2: raise SystemExit('Usage: rebuild.py NEW_OUTPUT_DIRECTORY')
...
with tempfile.TemporaryDirectory(prefix='.rebuild-',dir=final.parent) as temporary:
 out=Path(temporary)/'validated'
 out.mkdir()
 records={}
 for name,files,size in [(key,value['files'],value['size']) for key,value in cases.items()]:
  ...
  for key,relative in files.items():
   ...
   elif source.findall('.//fragment'):
    raise ValueError('conditions component contains chemical fragments')
   fmap={};cmap={}
  ...
  observed=document_inventory(target)
  if observed != expected: raise ValueError(name + ': assembly changed inventory')
